...
public class DeBuff_Core : CoreBase
{
    ...
    void Offense(GameObject target)
    {
        target.GetComponent<Tower>().AddBuff(buffData.addOffense, 0, 0);
    }

    void AttackSpeed(GameObject target)
    {
        target.GetComponent<Tower>().AddBuff(0, buffData.addAttackSpeed, 0);
    }

    void Range(GameObject target)
    {
        target.GetComponent<Tower>().AddBuff(0, 0, buffData.addRange);
    }
}
